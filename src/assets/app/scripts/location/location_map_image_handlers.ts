...
export function populateSubLocationMap(location: MapLocation) {
  const locationData = gLocationData[location] as GameLocation
  ...
  if (!mapOfSubLocations) {
    ...
    const coordArray = Object.values(
      locationData?.subLocations as GameSubLocation
    ).map(subLocationData => {
    ...
    for (const id in locationData.subLocations) {
      if (Object.prototype.hasOwnProperty.call(locationData.subLocations, id)) {
...
export function loadGameMap(
  ...
  defaultZoomLevel: number | null = null
) {
    ...
    const spawnPlayerSpriteOnMap = () => {
      ...
      newImgCopy.src =
        svgMainImg.attr('xlink:href') != undefined
          ? (svgMainImg.attr('xlink:href') as string)
          : (svgMainImg.attr('href') as string)
        ...
        if (defaultZoomLevel == null && shouldChangeDefaultZoomLevel) {
          ...
          if (roundedTransformValue > rawTransformValue) {
            ...
            if (isParentMapPopout)
              setMapPopoutZoomLvl(roundedTransformValue - 0.5)
          } else {
            element.css('transform', `scale(${roundedTransformValue})`)
            if (isParentMapPopout) setMapPopoutZoomLvl(roundedTransformValue)
          }
        } else {
          // Use the default zoom level given to scale
          element.css('transform', `scale(${defaultZoomLevel})`)
          if (isParentMapPopout) setMapPopoutZoomLvl(defaultZoomLevel ?? 1)
        }
      }
    }
