...
export class Fetus implements SugarBoxCompatibleClassInstance<SerializedFetus> {
	...
	static fromJSON(data: SerializedFetus): Fetus {
		return Object.assign(new Fetus(0), data);
	}
	...
	get growthRate() {
		...
		// biome-ignore lint/style/noNonNullAssertion: <This won't be undefined in practice>
	...
	static #getAccurateFetalStatForDevelopmentStage(
		...
		if (gestationalWeek < GestationalWeek.One) {
			...
			fetalStat =
				gestationalWeekStat +
				(Fetus.#getStatForGestationalWeekInOverduePregnancy(
	...
	static calcGrowthStatChange(
		...
		oldStat = Fetus.#getAccurateFetalStatForDevelopmentStage(stat, oldDevRatio);
		newStat = Fetus.#getAccurateFetalStatForDevelopmentStage(stat, newDevRatio);
